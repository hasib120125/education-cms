...
class AdministrationController extends Controller
{
    ...
    public function store(Request $request)
    {
      ...
      {
        $fPdfFile = $request->file('filePDF')->getClientOriginalExtension();
        $fFileName = pathinfo($request->file('filePDF')->getClientOriginalName(), PATHINFO_FILENAME);
        $fNewFileName = date('Y_m_d_h_i_s_') . $fFileName . '.' . $fPdfFile;
        $sDirectoryPath=$sDirectory.'/download/'.$fNewFileName;
        $sPath = Storage::disk('s3')->put($sDirectoryPath, $fPdfFile, 'public');
